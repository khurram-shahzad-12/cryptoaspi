...
const sectionMetatags = async (sec_id) => {
    try {
        var query = `SELECT seo_title,seo_description,seo_keywords FROM ourshopee_sections WHERE id=${sec_id} and country_id=1 order by position asc;`;
        var result = await db.runQuery(query);
        result = result[0];
        return result;
    } catch (err) {
...
const sectionMetatag = async (req) => {
    try {
...
const clearance_sale = async (req) => {
    try {
        const sec_id=117;
        const section_metatags = await sectionMetatags(sec_id);
        ...
        return { top_items: topItemsList, items: itemsList, bannerImages: bannerImages,meta_tags:section_metatags }
...
module.exports = {
    ...
    sectionMetatags: sectionMetatag,
}