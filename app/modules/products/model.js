...
//For datalayer
const product_detail_id_elk = async (req) => {
    const client = await config.elasticsearch();
    const result = await client.search({
        index: process.env.es_index,
        body: {
...
const product_detail_id = async (req) => {
    try {
        const elk_result = await product_detail_id_elk(req);
        const searched_products = elk_result
        return searched_products
    } catch (err) {
        var err_report = "{" + err.stack + "}"
