    ...
    try {
        ...
        if (result[0].length > 0) {
            ...
            return result[0].map(ele => {
                var odate = moment(ele.order_date).format('MM-DD-YYYY');
                var pdate = moment(odate).add(2, 'days').format('MM-DD-YYYY');
                var disdate = moment(pdate).add(1, 'days').format('MM-DD-YYYY');
                var deldate = moment(disdate).add(2, 'days').format('MM-DD-YYYY');
                var ordered_date = moment(odate).format('dddd').substring(0, 3) + ", " +  moment(odate).format("MMM Do"); 
                var packed_date = moment(pdate).format('dddd').substring(0, 3) + ", " +  moment(pdate).format("MMM Do"); 
                var dispatched_date = moment(disdate).format('dddd').substring(0, 3) + ", " +  moment(disdate).format("MMM Do"); 
                var delivery_date = moment(deldate).format('dddd').substring(0, 3) + ", " +  moment(deldate).format("MMM Do"); 
